...
class HybridEPBuffer:
    ...
    def dispatch(
        ...
        if handle is None:
            ...
            cached_num_dispatched_tokens = None
        else:
            (
                sparse_to_dense_map,
                rdma_to_attn_map,
                ...
                num_of_tokens,
                config,
                cached_num_dispatched_tokens,
            ) = handle

        if num_dispatched_tokens is None:
            if cached_num_dispatched_tokens is not None:
                # Recompute path: use cached value from the first forward pass to avoid CPU sync.
                num_dispatched_tokens = cached_num_dispatched_tokens
            else:
                # First forward pass: synchronize the stream to read num_dispatched_tokens from pinned memory.
        ...
        *,
        ...
        with torch.cuda.nvtx.range("hybrid-ep dispatch with permute phase"):
            ...
            if handle is None:
                ...
                cached_num_permuted_tokens = None
                cached_tokens_per_expert = None
            else:
                (
                    sparse_to_dense_map,
                    rdma_to_attn_map,
                    ...
                    config,
                    overflow_flag,
                    cached_num_permuted_tokens,
                    cached_tokens_per_expert,
                ) = handle
                if num_of_tokens_per_rank_in_handle != num_of_tokens_per_rank:
                    warnings.warn("This handle could be invalid.")
                # Use cached num_permuted_tokens from the first forward pass to avoid CPU sync in recompute.
                if num_permuted_tokens is None or num_permuted_tokens < 0:
                    num_permuted_tokens = cached_num_permuted_tokens
            ...
            if cached_num_permuted_tokens is None:
                if non_blocking:
                    cached_num_permuted_tokens = num_permuted_tokens
                else:
                    cached_num_permuted_tokens = int(tokens_per_expert.sum().item())
